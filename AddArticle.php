...
if(isset($_POST['addarticle'])){
    ...
    if (!empty($_SESSION['username']) && ($_SESSION['username'] == 'Admin')) {
        ...
        $artcontent = str_replace('tryit.asp?','TryIt.php?',$artcontent);
        $artcontent = str_replace('trysql.asp?','TrySQL.php?',$artcontent);
        $artcontent = str_replace('showphp.asp?','ShowPHP.php?',$artcontent);
    } else {
        $published = 2;
    }
    $author = $_SESSION['uid'];
    ...
    <div class="container">
        <div class="col-md-6" >
            ...
            <form method="post" name="artEditor" id="artEditor" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                ...
                Содержание:<br>
                <textarea rows="23" cols="70" name="artcontent" id="artcontent"></textarea><br>
<!--                TODO: кнопки для тегов и картинок             -->
                <input type="button" name="addImage" id="addImage" value="Добавить картинку" style="display: block; margin-bottom: 10px">
...
<div id="editArtList" style="display: none">
    ...
    if (!empty($_SESSION['username'])) {
        ...
        echo "<option value='NewArticle'>...Добавить статью...</option>";
        echo "</div>";
        echo "<div id='PHPArtList'>";
        $query = "SELECT `id`,`Name`,`Author` from `articles` WHERE `Topic`='PHP' ORDER BY `id`";
        $result = mysqli_query($conn, $query);
        while ($row = mysqli_fetch_array($result)) {
            if (($uid == $row['Author']) || ($uid == $admuid)) {
                $name = $row['Name'];
        ...
        echo "<option value='NewArticle'>...Добавить статью...</option>";
        echo "</div>";
        echo "<div id='MySQLArtList'>";
        $query = "SELECT `id`,`Name`,`Author` from `articles` WHERE `Topic`='MySQL' ORDER BY `id`";
        $result = mysqli_query($conn, $query);
        while ($row = mysqli_fetch_array($result)) {
            if (($uid == $row['Author']) || ($uid == $admuid)) {
                $name = $row['Name'];
