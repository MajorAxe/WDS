...
<script type="text/javascript">
    (function (d, w, c) {
        (w[c] = w[c] || []).push(function() {
            try {
                w.yaCounter36769855 = new Ya.Metrika({
                    id:36769855,
                    clickmap:true,
                    trackLinks:true,
...
<noscript><div><img src="https://mc.yandex.ru/watch/36769855" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
<!-- /Yandex.Metrika counter -->
<!--ОКНО ВХОДА-->
<div id="wrapper">
...
<!--                <label for="loginkeeping">Запомнить меня</label>-->
    ...
    if (isset($_SESSION['uid'])) {
        ...
        $JSArtCount = $row[0];
        $row = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM `articles` WHERE `Topic`='PHP'"));
        $PHPArtCount = $row[0];
        $row = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM `articles` WHERE `Topic`='MySQL'"));
        $MySQLArtCount = $row[0];
        ...
        $JSArtRead = $row[0];
        $row = mysqli_fetch_row(mysqli_query($conn, "SELECT DISTINCT COUNT(*) FROM `articlesread` WHERE `uid`='$uid' AND `topic`='PHP'"));
        $PHPArtRead = $row[0];
        $row = mysqli_fetch_row(mysqli_query($conn, "SELECT DISTINCT COUNT(*) FROM `articlesread` WHERE `uid`='$uid' AND `topic`='MySQL'"));
        $MySQLArtRead = $row[0];
        ...
        $PHPPercentage = round($PHPArtRead*100/$PHPArtCount);
        $MySQLPercentage = round($MySQLArtRead*100/$MySQLArtCount); //TODO: выровнять таблицу
//        $_SESSION['MessageForUser'] = $row[0];
        ...
        <div class='col-md-6' >
            <table class = 'StaticticsTable' style='text-align: right; padding: 10px; margin-left: 40px; margin-bottom: 10px'>
                ...
                <tr style='background-color: #84489c'>
                    <td>PHP</td>
                    <td>$PHPArtRead/$PHPArtCount</td>
                    <td>
                        <progress max='100' value='$PHPPercentage'></progress>
                    </td>
                    <td>$PHPPercentage%</td>
                </tr>
                <tr style='background-color: #e48e00'>
                    <td>MySQL</td>
                    <td>$MySQLArtRead/$MySQLArtCount</td>
                    <td>
                        <progress max='100' value='$MySQLPercentage'></progress>
                    </td>
                    <td>$MySQLPercentage%</td>
                </tr>
            </table>";?>
    ...
    echo "
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            if ($messread) {
                ...
                echo "<li class='singleMessageBox' id='Message$messid'>";
            } else if ($sendername == 'Admin') {
                $ArtStyle = 'CSSButtonRead';
                echo "<li class='singleMessageBox' id='Message$messid' onclick='markMessage($messid)'>";
            } else {
                $ArtStyle = 'JSButtonRead';
                echo "<li class='singleMessageBox' id='Message$messid' onclick='markMessage($messid)'>";
            }
            echo "
                <div class='thumbnail' id='$ArtStyle' style='padding: 8px; cursor: default;'>
                    <div style='display: inline-block;'>
                        <div class='messagehead'>$sendername   $messdate</div>
                        <div class='messagecont'>$messagetext</div>
                    </div>
                    <div style='display: inline-block; float: right; cursor: pointer' onclick='deleteMessage($messid)'><img src='images/close.png'></div>
                </div>
            </li>";
        }
        echo "
...
<div class="menu">
    ...
    <ul>
        ...
        <li><a href="PHPArticles.php">PHP</a></li>
        <li><a href="MySQLArticles.php">MySQL</a></li>
...
<script type="text/javascript" src="JS/topmenuanim.js"></script>
<script type="text/javascript" src="JS/RegisterChecks.js"></script>
<script src="JS/MessageWork.js"></script>
<?php
